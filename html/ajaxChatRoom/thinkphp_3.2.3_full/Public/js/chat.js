...
function listenEvent() {
    ...
    //有消息到来时触发
    ws.onmessage = function (e) {    //onmessage监听  postMessage接收
...
function selectUser(userid) {
    $('#userlist').val(userid);   //属性验证
}

function delUser(userid) {
    $('#user_' + userid).remove();  //移除元素
    $('#inroom_' + userid).remove();
    delete (userlist[userid]);   //删除元素
}
...
function showChatFace() {
    $("#chat_face").attr("class", "chat_face chat_face_hover");  //设置返回属性
...
function toggleFace() {
    $("#chat_face").toggleClass("chat_face_hover");     //oggleClass() 对设置或移除被选元素的一个或多个类进行切换。
